...
import ru.kode.impl.config.DebugItem
...
import ru.kode.plexus.toFeatureValue
    ...
    setContent {
      ...
      LaunchedEffect(Unit) {
        plexusManager.getFeaturesValue(
          ...
          debugFeatures = features.map { (appFeature, value) ->
            DebugItem(
              key = appFeature.key,
              featureValue = value.toFeatureValue()
            )
          }
        }.launchIn(scope)
      }
      ...
      Column(
        ...
        debugFeatures.forEach { item ->
          when (item.featureValue) {
            is FeatureValue.DoubleValue -> {
              DoubleItem(
                name = item.key,
                currentValue = item.featureValue.value,
                values = remember { listOf(25.0, 50.0, 75.0, 100.0) },
                onValueChange = { debugConfig.setFeatureEnabledState(item.key, FeatureValue.DoubleValue(it)) }
              )
            }
            is FeatureValue.LongValue -> {
              LongItem(
                name = item.key,
                currentValue = item.featureValue.value,
                values = remember { listOf(25L, 50L, 75L, 100L) },
                onValueChange = { debugConfig.setFeatureEnabledState(item.key, FeatureValue.LongValue(it)) }
              )
            }
            is FeatureValue.BooleanValue -> {
              BooleanItem(
                name = item.key,
                checked = item.featureValue.value,
                onCheckedChange = { debugConfig.setFeatureEnabledState(item.key, FeatureValue.BooleanValue(it)) }
              )
            }

            is FeatureValue.StringValue -> {
              var temp by remember(item.featureValue.value) { mutableStateOf(item.featureValue.value) }
              StringItem(
                name = item.key,
                value = temp,
                onApply = { debugConfig.setFeatureEnabledState(item.key, FeatureValue.StringValue(temp)) },
